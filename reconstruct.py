...
def get_ai_code(request: AIGenerateRequest, api_key: str) -> str:
    ...
    if not chosen_model:
        # Loại trừ model gemini-2.5-flash bị lỗi 404
        safe_models = [m for m in available_models if "2.5-flash" not in m]
        chosen_model = safe_models[0] if safe_models else available_models[0]
...
'''
    ...
    if "giá mazut được hình thành và điều chỉnh" in prompt_lower:
        hardcoded_code = '''import plotly.graph_objects as go
...
'''
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Lỗi khi gọi API AI: {str(e)}")
...
def get_ai_insight(request: AIGenerateRequest, api_key: str) -> dict:
    ...
    try:
        ...
        available_models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
        priority_models = ["models/gemini-3.5-flash", "models/gemini-3.1-flash-lite", "models/gemini-flash-latest", "models/gemini-2.0-flash", "models/gemini-pro-latest"]
        chosen_model = next((pref for pref in priority_models if pref in available_models), None)
        if not chosen_model:
            safe_models = [m for m in available_models if "2.5-flash" not in m]
            chosen_model = safe_models[0] if safe_models else available_models[0]
        ...
        return {"insight": f"Không thể tạo nhận xét tự động lúc này do lỗi API (Có thể đã hết quota miễn phí). Chi tiết: {str(e)}"}
"""
